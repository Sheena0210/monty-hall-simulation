...
#(3-1)如果主持人知道跑車在哪，比較三種情況（r內建、inverse method、antithetic）sample mean分布----
# R 內建 Bernoulli
...
#(3-2)如果主持人不知跑車在哪，比較三種情況（r內建、inverse method、antithetic）sample mean分布----
# R 內建 Bernoulli
R_default_unknown_distribution <- function(n = 1000, B = 1000, p = 1/3){
  y <- numeric(B)
  for(i in 1:B){
    x <- rbinom(n, size = 1, prob = p)
...
# inverse function
Inverse_unknown_distribution <- function(n = 1000, B = 1000, p = 1/3){
  y <- numeric(B)
...
# inverse function + controlling var
Control_unknown_distribution <- function(n = 1000, B = 1000, p = 1/3){
  y <- numeric(B)
...
R_default_known_y <- function(n = 1000, B = 1000, p = 1/3){
  y <- numeric(B)
  for(i in 1:B){
    x <- rbinom(n, size = 1, prob = p)
    result <- change_known(x)
...
# Inverse function
Inverse_known_y <- function(n = 1000, B = 1000, p = 1/3){
  y <- numeric(B)
  for(i in 1:B){
    x <- first_car(n, p)
    result <- change_known(x)
...
Control_known_y <- function(n = 1000, B = 1000, p = 1/3){
  y <- numeric(B)
  for(i in 1:B){
    x <- first_car_control_var(n, p)
    result <- change_known(x)
...
R_default_unknown_distribution  <-R_default_unknown_distribution(n, B, p)
hist(R_default_unknown_distribution ,
     main = "Sampling Distribution of the Estimated Winning Probability 
     (R Bernoulli Simulation)",
     xlab = "Estimated winning probability",
     probability = TRUE)

abline(v = mean(R_default_unknown_distribution ),
       col = "yellow",
       lty = 2,
       lwd = 2)
text(x = mean(R_default_unknown_distribution ) + 0.002,
     y = max(hist(R_default_unknown_distribution , plot = FALSE)$density) * 0.9,
     labels = paste0("Mean = ", round(mean(R_default_unknown_distribution ), 4)),
     pos = 4)


# Inverse function
Inverse_unknown_y <- function(n = 1000, B = 1000, p = 1/3){
  y <- numeric(B)
  for(i in 1:B){
    x <- first_car(n, p)
    result <- change_unknown(x)
...
text(x = mean(Inverse_unknown_y) + 0.002,
     ...
     pos = 4)
var(Inverse_unknown_y)

# Inverse function + antithetic variates
Control_unknown_y <- function(n = 1000, B = 1000, p = 1/3){
  y <- numeric(B)
  for(i in 1:B){
    x <- first_car_control_var(n, p)
    result <- change_unknown(x)
